...
import module namespace display="http://marklogic.lib.uchicago.edu:8003/display" at
"display.xqy"

define variable $collection {"archival_photofiles"}

(: if input contains quotation marks, it returns no records found if
   the records don't also contain the quoted input. so strip them. :)

define function get-user-input ($input) {
replace(normalize-space(xdmp:get-request-field($input)), """", "")
}
...
let $results := 
    for $d in cts:search(collection("archival_photofiles")/record, 
...
{
for $group at $g in $groups
for $record at $r in $records
where fn:substring($record/v, 1, 1) = $group
order by if ($b = 'browse7.xml')
         then browsesort($record/v)
         else $record/v
return if ($r = 1)
       then <r id="{$r}" a="{$group}">{$record/v, $record/c}</r>
       else <r id="{$r}">{$record/v, $record/c}</r>
}
</list>
}
